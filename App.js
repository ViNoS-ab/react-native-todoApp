   ...
   <View style={styles.container}>  
	...
	My Tasks 
     ...
     <View style={styles.addingContainer}> 
      ...
      <FlatList
	          data={todos}
	          renderItem={ ({item}) => <Todo remove={()=> remove(item.id)}  todo={item} />}
...
const Todo =  ({ todo: {id , text}, remove }) => {
 ...
 <View style={styles.todo}>
  <View style={{display: "flex", flexDirection: "row"}} >
   <Pressable onPress={() => setChecked(!checked)} style={[styles.checkbox,checked && styles.checkboxChecked]} ></Pressable>
   <Text style={[{color: "black"} , checked && {textDecorationLine: "line-through", color: "grey"}] } >{text}</Text> 
  </View>
